...
* Check the `ConnectionStrings` in `appsettings.json` files under the `abp_obs_project.Blazor` and `abp_obs_project.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 239a149f-3f84-4c18-941e-c8274e50fe42
```

> `239a149f-3f84-4c18-941e-c8274e50fe42` is the password of the certificate, you can change it to any password you want.
...
Run `abp_obs_project.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `abp_obs_project.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `abp_obs_project.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
